...
if [[ ! -d "$API_SPEC_PATH" ]]; then
  ...
  git clone git@github.com:snyk/breakability-service.git $API_SPEC_PATH
fi
...
# Vendor OpenAPI build artefacts for use in the Breakability API client
cp $API_SPEC_PATH/internal/api/hidden/versions/$VERSION/spec.yaml ./internal/apiclients/breakability/$VERSION/spec.yaml
